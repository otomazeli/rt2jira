...
def format_search_results(jira_results):
    ...
    for result in jira_results:
        ...
        entry = template % {"jira_url_prefix": config.get('jira', 'jira_url_prefix'), "issue_key": result.key,
                            "issue_summary": result.fields.summary, "issue_status": result.fields.status}
        output = output + '\n' + entry
...
jira_results = jira.search_issues(
    'project = "' + config.get('jira', 'project') + '" AND component = "' + config.get('jira',
                                                                                       'component') + '" AND labels in ("' + config.get(
        'jira', 'new_issue_label') + '", "' + config.get('jira', 'new_comment_label') + '") ORDER BY created ASC',
    maxResults=False)
...
try:
    for jira_issue in jira_results:
        ...
        if sanitized_summary:
            ...
            related_jira_query = 'key != ' + jira_issue.key + ' AND project = ' + config.get('jira',
                                                                                             'project') + ' AND component = "' + config.get(
                'jira', 'component') + '" AND text ~ "' + sanitized_summary + '" ORDER BY updated DESC'
            related_jira_results = jira.search_issues('level IS EMPTY AND ' + related_jira_query,
                                                      maxResults=config.getint('jira', 'auto_reply_max_results'))

        # Recent search results.
        recent_jira_query = 'key != ' + jira_issue.key + ' AND project = ' + config.get('jira',
                                                                                        'project') + ' AND component = "' + config.get(
            'jira', 'component') + '" ORDER BY created DESC'
        recent_jira_results = jira.search_issues('level IS EMPTY AND ' + recent_jira_query,
                                                 maxResults=config.getint('jira', 'auto_reply_max_results'))
        ...
        reply_message = template_issue.fields.description % {"jira_url_prefix": config.get('jira', 'jira_url_prefix'),
                                                             "issue_key": jira_issue.key,
                                                             "issue_summary": jira_issue.fields.summary,
                                                             "related_results": format_search_results(
                                                                 related_jira_results),
                                                             "recent_results": format_search_results(
                                                                 recent_jira_results),
        ...
        if len(requestors) > 0:
            ...
            logger.info(
                'Adding new comment to (' + jira_issue.key + ') to track existing notifiers (' + cc_addresses + ')')
